...
class TestElementsAsync:
    async def test_get_by_path(self, async_client):
        ...
        elem = await client.elements.get_by_path(r"\\AF\Production\Pump-001")
    ...
    async def test_get_by_web_id(self, async_client):
        ...
        elem = await client.elements.get_by_web_id(ELEM_WEB_ID)
        assert elem.web_id == ELEM_WEB_ID

    async def test_get_children(self, async_client):
        client, mock = async_client
        mock.get(f"/elements/{ELEM_WEB_ID}/elements").mock(
            return_value=httpx.Response(200, json={"Items": [ELEMENT_MOTOR]})
        )
        children = await client.elements.get_children(ELEM_WEB_ID)
